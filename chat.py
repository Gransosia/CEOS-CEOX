...
class ConversationalEngine:
    ...
    def _is_literature_query(self, t: str) -> bool:
        # No tratar «me llamo…» / identidad como consulta literaria
        if re.match(r"(?i)^\s*(me\s+llamo|mi\s+nombre\s+es|soy\s+[A-ZÁÉÍÓÚ]|ll[aá]mame|identif)", t or ""):
            return False
        return bool(re.search(
            r"\b(libro|libros|autor|novela|cuento|estilo|literar|prosa|narrat|personaje|"
            r"capítulo|capitulo|obra|virtud|defect|crítica|critica|voz narrativa)\b",
            t,
            re.I,
        ))
    ...
    def _local_reply(self, user_text: str, history: list, context: str) -> str:
        ...
        # Evitar volcar doctrina CRONOS ante preguntas genéricas o de uso
        if not re.search(r"(?i)\b(cronos|protocolo|espiral|membrana|r[eé]gimen|arquetipo)\b", t):
            try:
                frags = self.mentor._pick_knowledge(raw, n=2) if hasattr(self.mentor, "_pick_knowledge") else []
                # filtrar fragmentos de doctrina pura si la pregunta no es de protocolo
                clean_frags = []
                for f in frags or []:
                    fs = str(f)
                    if re.search(r"(?i)cronos representa|espiral es un algoritmo|todo sistema adaptativo", fs):
                        continue
                    clean_frags.append(fs)
                if clean_frags:
                    joined = "\n\n".join(str(f)[:220] for f in clean_frags)
                    return (
                        f"{hello}Con el material que manejo:\n\n"
                        + joined
    ...
    def reply(
        ...
        *,
        ...
        hub = None
        try:
            from .hub_intents import handle_hub_intent
            from .research import research_topic as _research_topic

            def _res_fn(topic: str):
                return _research_topic(topic, focus=None, library=self.library)

            hub = handle_hub_intent(
                user_text,
                user=self.user,
                library=self.library,
                research_fn=_res_fn,
            ...
            hub = None
        # Fallback mínimo si el módulo hub no está desplegado
        if not hub:
            low0 = user_text.lower().strip()
            mname = re.match(
                r"(?i)^\s*(?:me\s+llamo|mi\s+nombre\s+es|puedes\s+llamarme|ll[aá]mame)\s+(.+?)\s*$",
                user_text,
            )
            if mname and self.user is not None:
                try:
                    nm = mname.group(1).strip(" .,!")
                    if 1 < len(nm) < 60:
                        self.user.set_name(nm)
                        hub = {
                            "ok": True,
                            "reply": f"Perfecto, te recordaré como **{nm}**. Ya puedes subir docs con 📎 o preguntarme por tu material.",
                            "engine": "hub",
                            "mode": "identity",
                            "web": False,
                        }
                    ...
                    pass
            elif re.search(r"(?i)qu[eé]\s+(hay|tienes).*(reservorio|libros|documentos)|lista(r)?\s+(el\s+)?reservorio", low0):
                try:
                    from .hub_intents import library_list_text
                    hub = {"ok": True, "reply": library_list_text(self.library), "engine": "hub", "mode": "library", "web": False}
                except Exception:
                    docs = []
                    try:
                        docs = self.library.list_docs() if self.library else []
                    except Exception:
                        docs = []
                    if not docs:
                        hub = {
                            "ok": True,
                            "reply": "El reservorio está vacío. Usa 📎 en este chat para subir PDF/DOCX/TXT.",
                            "engine": "hub",
                            "mode": "library",
                            "web": False,
                        }
                    else:
                        lines = [f"Reservorio: {len(docs)} documento(s)."]
                        for d in docs[-15:]:
                            lines.append("· " + (d.get("title") or d.get("source_name") or "?"))
                        hub = {"ok": True, "reply": "\n".join(lines), "engine": "hub", "mode": "library", "web": False}
            elif re.search(r"(?i)qu[eé]\s+puedes\s+hacer|c[oó]mo\s+te\s+uso|ayuda(?:me)?$", low0):
                hub = {
                    "ok": True,
                    "reply": (
                        "Desde este chat puedes: (1) identificarte («Me llamo …»), "
                        "(2) subir documentos con 📎, (3) listar el reservorio, "
                        "(4) pedir «Investiga …» o hablar de tus textos, (5) «Qué has aprendido». "
                        "Las otras pestañas son opcionales."
                    ),
                    "engine": "hub",
                    "mode": "help",
                    "web": False,
        ...
        if hub and hub.get("ok"):
            ...
            history.append({"role": "assistant", "content": hub.get("reply") or "", "ts": _now()})
            session["messages"] = history[-80:]
            session["updated_at"] = _now()
            try:
                self.store.save(session)
            except Exception:
        ...
        if llm_result.get("ok"):
            ...
            hint = str(llm_result.get("hint") or llm_result.get("error") or "")
            err = llm_result.get("error")
            if hint and err not in (None, "sin_api", ""):
                if "403" in hint or "Forbidden" in hint:
                    tip = (
                        "\n\n⚠️ Groq rechazó la clave (403). "
                        "Regenera API key en console.groq.com y actualiza GROQ_API_KEY en Render → Environment."
                    )
                else:
                    tip = "\n\n(Nota: LLM no disponible; respuesta local. " + hint[:120] + ")"
                answer = answer + tip

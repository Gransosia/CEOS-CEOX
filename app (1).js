...
async function uploadFilesFromChat(fileList) {
  ...
  // Preferir un solo POST con todos los archivos (se acumulan en el índice)
  const fdAll = new FormData();
  if (author) fdAll.append("author", author);
  files.forEach((file) => fdAll.append("file", file, file.name));
  try {
    const res = await fetch("/api/mentor/ingest/file", {
      method: "POST",
      headers: { "X-Device-Id": getDeviceId() },
    ...
    const data = await res.json().catch(() => ({}));
    lastStats = data.library_stats || null;
    (data.results || []).forEach((r) => {
      if (r.status === "ok") {
        okN++;
        lines.push("✓ " + (r.file || "?") + " (" + (r.chunks || 0) + " trozos)");
      } else {
        lines.push("✗ " + (r.file || "?") + ": " + (r.error || "error"));
      }
    });
    if (!(data.results || []).length && data.ok === false) {
      lines.push("✗ " + (data.error || res.status));
    }
  } catch (e) {
    // Fallback: uno a uno si el lote falla
    for (let i = 0; i < files.length; i++) {
      const file = files[i];
  ...
  const summary =
    ...
    "\n" + lines.join("\n") +
    "\n\nCada tanda se suma a la anterior (no sustituye). " +
    "En Render free: Sync → Descargar copia completa para no perder el reservorio.";
  setHubStatus(
    okN
      ? "Listo +" + okN + (total != null ? " · total " + total : "")
      : "Falló la subida"
  );

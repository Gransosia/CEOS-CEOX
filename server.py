...
def api_sync_status():
    ...
    try:
        mem = get_memory()
        out["cases"] = len(mem.cases() or [])
        out["hitos"] = len(mem.hitos() or [])
        out["trajectories"] = len(mem.trajectories() or [])
    except Exception as e:
        out["memory_error"] = str(e)[:120]
    try:
        out["grammar"] = get_grammar().stats()
    except Exception as e:
        out["grammar_error"] = str(e)[:120]
    try:
        out["library"] = get_library().stats()
    except Exception as e:
            ...
            try:
                if suffix in ARCHIVE_SUFFIXES:
                    ...
                    results.append({
                        ...
                        "kind": entry.get("kind"),
                        "library_docs_total": entry.get("library_docs_total"),
                    })
            except Exception as e:
...
def api_chat():
    """Chat a prueba de fallos: nunca devolver HTML 500 opaco."""
    import traceback as _tb
    try:
        data = request.get_json(force=True) or {}
    except Exception:
    ...
    if not text:
        return jsonify({"ok": False, "error": "mensaje vacío", "reply": "Escribe un mensaje para continuar."}), 400
    sid = data.get("session_id") or "default"
    device = request.headers.get("X-Device-Id")
    long_mode = bool(data.get("long") or data.get("version_larga") or data.get("larga"))
    # Por defecto NO buscar web (solo si el cliente lo pide)
    web_raw = data.get("web", data.get("internet", False))
    web_mode = False if web_raw in (False, "false", "0", 0, None) else True
    try:
        engine = get_chat()
        result = engine.reply(sid, text, device_id=device, long=long_mode, web=web_mode)
        if not isinstance(result, dict):
            result = {"ok": True, "reply": str(result), "engine": "local"}
        result.setdefault("ok", True)
        if not result.get("reply"):
            result["reply"] = result.get("error") or "No pude generar respuesta en este turno."
        return jsonify(result)
    except Exception as e:
        err = str(e)[:300]
        return jsonify({
            ...
            "error": err,
            "reply": (
                "Ha fallado el motor de chat en este turno. "
                "Prueba un mensaje más corto o recarga la página. "
                f"Detalle: {err}"
            ),
            "engine": "error",
            "mode": "error",
...
def api_chat_sessions():
    try:
        ...
        return jsonify({"ok": False, "sessions": [], "error": str(e)[:200]})

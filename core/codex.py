...
# Marcos de expansión para "leer" un prisma de muchas formas distintas a
# partir de muy pocos datos guardados (átomos + fórmula + semilla).
PRISM_EXPANSION_FRAMES = [
    "Definición nuclear: {seed} Esto se sostiene en {atoms_desc}.",
    "Aplicación práctica: si observas {atoms_desc} en un sistema real, la señal reveladora es «{seed}».",
    "Tensión: {seed} — el riesgo aparece cuando se ignora la relación {formula}.",
    "Pregunta para el alumno: dado que {seed}, ¿cómo distinguirías {atom0} de {atom1} en tu propio caso?",
    "Síntesis breve: {formula} resume que {seed}",
    "Contraejemplo a vigilar: si {atom0} aparece sin {atom1}, sospecha que «{seed}» se está aplicando mal.",
]
...
class Codex:
    ...
    # ----- Prisma: LLM (si hay) o heurística → átomos + fórmula + semilla -----
    def crystallize_prism(self, text: str, topic: str, use_llm: bool = True) -> dict:
        """
        Reduce un texto (potencialmente largo) a un 'prisma': hasta 4 átomos
        del alfabeto FIJO (ATOMOS) + una fórmula molecular + una frase-semilla
        (<220 caracteres). Es deliberadamente con pérdida: el soporte guardado
        es minúsculo (unas pocas decenas de bytes) frente al texto original.

        No es "encriptar el LLM": el LLM (si hay clave configurada) se usa
        UNA VEZ, para destilar el texto a ese vocabulario cerrado. Lo que se
        guarda no depende ya de ningún LLM. La riqueza al releer viene de
        combinar ese puñado de piezas con los marcos de expand_prism(), no
        de que el prisma "sepa" más de lo que se le metió.
        """
        text = (text or "").strip()
        if not text:
            return {"ok": False, "error": "texto vacío"}

        atoms, formula, seed, engine = None, None, None, "heuristico"

        if use_llm:
            try:
                from .llm_bridge import available as llm_available, deep_write
                info = llm_available()
                if info.get("available"):
                    atom_list = "\n".join(f"{k}: {v}" for k, v in ATOMOS.items())
                    prompt = (
                        "Reduce el siguiente texto a como máximo 4 códigos de este "
                        f"alfabeto FIJO (usa solo estos códigos, no inventes otros):\n{atom_list}\n\n"
                        "Responde SOLO con un JSON de esta forma exacta, sin explicación ni texto extra:\n"
                        '{"atoms": ["COD1","COD2"], "formula": "COD1+COD2", '
                        '"seed": "una frase densa de menos de 220 caracteres con la idea nuclear"}\n\n'
                        f"TEXTO:\n{text[:4000]}"
                    )
                    res = deep_write(prompt, context="", max_tokens=300)
                    if res.get("ok"):
                        raw = res.get("text", "") or ""
                        m = re.search(r"\{.*\}", raw, re.S)
                        if m:
                            data = json.loads(m.group(0))
                            cand_atoms = [a for a in data.get("atoms", []) if a in ATOMOS][:4]
                            if cand_atoms and data.get("seed"):
                                atoms = cand_atoms
                                formula = data.get("formula") or "+".join(atoms)
                                seed = str(data.get("seed"))[:220]
                                engine = res.get("engine", "llm")
            except Exception:
                pass  # cae al heurístico; nunca rompe el flujo por falta de LLM

        if not atoms:
            # Fallback sin LLM (o si el parseo de su respuesta falló): coincidencia
            # léxica simple contra el propio alfabeto. Más pobre, pero funciona
            # sin clave ni GPU, como exige el stack.
            low = text.lower()
            hits = []
            for code, meaning in ATOMOS.items():
                key = meaning.split("—")[0].strip().lower()
                if key and (key in low or code.lower() in low):
                    hits.append(code)
            if not hits:
                hits = ["CMP", "TEN"]
            atoms = hits[:4]
        ...
        packed = json.dumps(prism, ensure_ascii=False)
        cid = self.store_crystal(packed, kind="prism", meta={"topic": topic})
        self.add_molecule(formula, seed, tags=[topic] if topic else [])
        ratio = round(len(text) / max(1, len(packed)), 1)
        return {"ok": True, "id": cid, "prism": prism, "compression_ratio": ratio}

    def expand_prism(self, cid: str, n: int = 3) -> dict:
        """
        'Lee' un prisma guardado y lo despliega en n desarrollos distintos,
        combinando sus átomos/fórmula/semilla con marcos de expansión fijos.

        La "casi infinitud" es combinatoria y honesta: nº de marcos × nº de
        órdenes posibles de átomos, no magia. Con 6 marcos y 4 átomos ya hay
        decenas de lecturas distintas de la MISMA pieza guardada.
        """
        crystal = self.get_crystal(cid)
        if not crystal or crystal.get("kind") != "prism":
            return {"ok": False, "error": "no es un cristal-prisma válido"}
        try:
            prism = json.loads(crystal["text"])
        except Exception:
            return {"ok": False, "error": "prisma corrupto"}

        atoms = prism.get("atoms") or ["CMP"]
        seed = prism.get("seed", "")
        formula = prism.get("formula", "+".join(atoms))
        atoms_desc = ", ".join(f"{a} ({ATOMOS.get(a, a)})" for a in atoms)

        import random
        frames = list(PRISM_EXPANSION_FRAMES)
        random.shuffle(frames)
        developments = []
        for frame in frames[:max(1, min(n, len(frames)))]:
            txt = frame.format(
                seed=seed,
                atoms_desc=atoms_desc,
                formula=formula,
                atom0=atoms[0] if atoms else "?",
                atom1=(atoms[1] if len(atoms) > 1 else (atoms[0] if atoms else "?")),
            )
            developments.append(txt)

        combinatorial_space_approx = len(PRISM_EXPANSION_FRAMES) * max(1, len(atoms))
        return {
            "ok": True,
            "id": cid,
            "prism": prism,
            "developments": developments,
            "combinatorial_space_approx": combinatorial_space_approx,
        }
